...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-2">
                        @php
                            $activeLeases = \App\Models\VehicleLease::where('status', 'active')->count();
                        @endphp
                        {{ $activeLeases }}
                    </p>
                    <a href="{{ route('admin.leasing.index') }}" class="text-sm text-green-600 hover:text-green-700 mt-2 inline-block">View All →</a>
                </div>
                <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">
                    <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Featured Leases</p>
                    <p class="text-2xl font-bold text-gray-900 mt-2">
                        @php
                            $featuredLeases = \App\Models\VehicleLease::where('is_featured', true)->count();
                        @endphp
                        {{ $featuredLeases }}
                    </p>
                    <p class="text-sm text-gray-500 mt-2">Special offers</p>
                </div>
                <div class="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Avg. Monthly Payment</p>
                    <p class="text-2xl font-bold text-gray-900 mt-2">
                        @php
                            $avgPayment = \App\Models\VehicleLease::where('status', 'active')->avg('monthly_payment');
                        @endphp
                        ${{ number_format($avgPayment ?? 0, 0) }}
                    </p>
                    <p class="text-sm text-gray-500 mt-2">Active leases</p>
                </div>
                <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
                    <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600">Reserved Leases</p>
                    <p class="text-2xl font-bold text-gray-900 mt-2">
                        @php
                            $reservedLeases = \App\Models\VehicleLease::where('status', 'reserved')->count();
                        @endphp
                        {{ $reservedLeases }}
                    </p>
                    <p class="text-sm text-orange-600 mt-2">In progress</p>
                </div>
