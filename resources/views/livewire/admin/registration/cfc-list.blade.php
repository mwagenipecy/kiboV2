    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 mb-6">
        <div class="border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between px-6 py-4 gap-4">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">CFC Registrations</h2>
                    <p class="text-sm text-gray-600 mt-1">Manage Commercial Finance Company registrations</p>
                </div>
                
                <a 
                    href="{{ route('admin.registration.cfcs.create') }}"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($cfcs as $cfc)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">#{{ $cfc->id }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900">{{ $cfc->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $cfc->email }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $cfc->phone_number }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-mono">{{ $cfc->registration_number }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($cfc->status === 'active')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @elseif ($cfc->approval_status === 'rejected')
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    @if ($cfc->approval_status === 'pending')
                                        <button 
                                            wire:click="confirmApprove({{ $cfc->id }})" 
                                            wire:loading.attr="disabled"
                                            wire:target="confirmApprove({{ $cfc->id }})"
                                            ...
                                            title="Approve"
                                        >
                                            <svg wire:loading.remove wire:target="confirmApprove({{ $cfc->id }})" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <svg wire:loading wire:target="confirmApprove({{ $cfc->id }})" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                            </svg>
                                        </button>
                                        <button 
                                            wire:click="reject({{ $cfc->id }})" 
                                            wire:loading.attr="disabled"
                                            wire:target="reject({{ $cfc->id }})"
                                            ...
                                            title="Reject"
                                        >
                                            <svg wire:loading.remove wire:target="reject({{ $cfc->id }})" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <svg wire:loading wire:target="reject({{ $cfc->id }})" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                            </svg>
                                        </button>
                                    @endif
                                    <a 
                                        href="{{ route('admin.registration.cfcs.edit', $cfc->id) }}" 
                                    ...
                                    <button 
                                        wire:click="confirmDelete({{ $cfc->id }})" 
                                        wire:loading.attr="disabled"
                                        wire:target="confirmDelete({{ $cfc->id }})"
                                        ...
                                        title="Delete"
                                    >
                                        <svg wire:loading.remove wire:target="confirmDelete({{ $cfc->id }})" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                        </svg>
                                        <svg wire:loading wire:target="confirmDelete({{ $cfc->id }})" class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                    </button>
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-sm">No CFCs found. Click "Add CFC" to create one.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                ...
                <div class="px-6 py-4 bg-white">
                    <div class="flex items-start">
                        ...
                        <div class="mt-0 ml-4 text-left">
                            <h3 class="text-lg font-medium text-gray-900">Approve CFC Registration</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to approve this CFC registration? This will create a user account and send credentials via email.
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full z-50">
                <div class="px-6 py-4 bg-white">
                    <div class="flex items-start">
                        ...
                        <div class="mt-0 ml-4 text-left">
                            <h3 class="text-lg font-medium text-gray-900">Delete CFC</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to delete this CFC? This action cannot be undone.
                                </p>
                            </div>
                        </div>
                    </div>
