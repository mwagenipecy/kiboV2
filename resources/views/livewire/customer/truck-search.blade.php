<div class="min-h-screen bg-white">
    {{-- Filter Bar: sticky top-0 so no gap; padding-top so content sits just below navbar; z-40 so navbar/dropdown stay on top; centered with rounded corners --}}
            ...
            <div class="flex items-center gap-3 justify-between flex-wrap">
                ...
                <div class="flex items-center gap-3 flex-wrap">
                    ...
                    @if($make)
                        ...
                        @endphp
                        <button wire:click="$toggle('showFilters')" class="hidden md:inline-flex px-6 py-2 bg-green-600 text-white rounded-full font-medium hover:bg-green-700 transition-colors">
                            {{ $makeName }}
                        </button>
                    @endif
                    ...
                    {{-- Price, Year, Mileage, Gearbox, Body type (hidden on mobile) --}}
                    ...
                    {{-- Mobile only: single chip (Used when no condition set) --}}
                    @if(!$condition)
...
<div>
    ...
    {{-- Filter modal animations (always in DOM so classes work when modal opens) --}}
    <style>
            ...
            <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col">
                ...
                <div class="relative aspect-[4/3] bg-gray-100 group" data-carousel="truck-{{ $truck->id }}">
                    @if($imageCount > 0)
                        ...
                        <a href="{{ route('trucks.detail', $truck->id) }}" class="carousel-image absolute inset-0 {{ $index === 0 ? '' : 'hidden' }}" data-index="{{ $index }}">
                            <img src="{{ asset('storage/' . $image) }}" alt="{{ $truck->full_name }}" class="w-full h-full object-cover">
                        </a>
                        @endforeach
                    @else
                        <a href="{{ route('trucks.detail', $truck->id) }}" class="absolute inset-0">
                    ...
                    {{-- Save Button --}}
                    <button wire:click.prevent="toggleSave({{ $truck->id }})" class="absolute top-2 right-2 w-10 h-10 bg-white rounded-full flex items-center justify-center hover:bg-gray-50 shadow-md transition-colors z-10">
                        <svg class="w-5 h-5 {{ in_array($truck->id, $savedTrucks) ? 'fill-red-500 text-red-500' : 'text-gray-600' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Content --}}
                <a href="{{ route('trucks.detail', $truck->id) }}" class="p-4 flex flex-col flex-grow">
                    <h3 class="text-base font-bold text-gray-900 mb-1">{{ $truck->make->name ?? '' }} {{ $truck->model->name ?? '' }}</h3>
                    <p class="text-sm text-gray-700 mb-1 line-clamp-2">{{ $truck->variant }}</p>
                    <p class="text-xs text-gray-600 mb-3">{{ $truck->year }} • {{ number_format($truck->mileage) }} km</p>
                ...
                <div class="px-4 pb-4 space-y-2" x-data="{ showMenu: false }">
                    ...
                    <div class="relative">
                        ...
                        <div x-show="showMenu" @click.away="showMenu = false" x-transition class="absolute bottom-full mb-2 left-0 right-0 bg-white rounded-lg shadow-xl border border-gray-200 z-20 overflow-hidden">
                            {{-- Valuation Report --}}
                            @auth
                            <button wire:click="openValuationModal({{ $truck->id }})" @click="showMenu = false" class="w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors flex items-start gap-3 border-b border-gray-100">
                            ...
                            {{-- Financing --}}
                            <button wire:click="openFinancingModal({{ $truck->id }})" @click="showMenu = false" class="w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors flex items-start gap-3 border-b border-gray-100">
                            ...
                            {{-- Cash Purchase --}}
                            <button wire:click="openCashPurchaseModal({{ $truck->id }})" @click="showMenu = false" class="w-full px-4 py-3 text-left hover:bg-gray-50 transition-colors flex items-start gap-3">
