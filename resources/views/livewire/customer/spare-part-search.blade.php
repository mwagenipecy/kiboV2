        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <input
                    ...
                    placeholder="Search by name, company, or parts..."
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($suppliers as $supplier)
                <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    ...
                    {{-- Supplier Info --}}
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $supplier->company_name ?? $supplier->name }}</h3>
                        <p class="text-sm text-gray-600 mb-3">{{ $supplier->name }}</p>
                        ...
                        {{-- Distance --}}
                        @if($supplier->distance !== null)
                            <div class="flex items-center gap-2 mb-3">
                                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path>
                                </svg>
                                <span class="text-sm font-medium text-green-600">{{ $supplier->distance }} km away</span>
                            </div>
                        @endif

                        {{-- Vehicle Makes --}}
                        @if($supplier->vehicle_makes && count($supplier->vehicle_makes) > 0)
                            <div class="mb-3">
                                <p class="text-xs text-gray-500 mb-1">Specializes in:</p>
                                <div class="flex flex-wrap gap-1">
                                    @php
                                        $makes = \App\Models\VehicleMake::whereIn('id', $supplier->vehicle_makes)->get();
                                    @endphp
                        ...
                        @if($supplier->spare_part_details)
                            <div class="mb-4">
                                <p class="text-xs text-gray-500 mb-1">Parts available:</p>
                                <p class="text-sm text-gray-700 line-clamp-2">{{ Str::limit($supplier->spare_part_details, 100) }}</p>
                            </div>
                        @endif
                        ...
                        <div class="flex gap-2">
                            @if($supplier->phone_number)
                                <a href="tel:{{ $supplier->phone_number }}" class="flex-1 px-4 py-2 bg-white border-2 border-green-600 text-green-600 text-center font-semibold rounded-lg hover:bg-green-50 transition-colors">
                                    Call
                                </a>
                            @endif
                            <a href="{{ route('spare-parts.supplier', $supplier->id) }}" class="flex-1 px-4 py-2 bg-green-600 text-white text-center font-semibold rounded-lg hover:bg-green-700 transition-colors">
                                View Details
                            </a>
                        </div>
                    </div>
    ...
    @if($showLocationModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" wire:click="showLocationModal = false">
            <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4" wire:click.stop>
                ...
                <p class="text-gray-600 mb-4">We need your location to show suppliers near you. Please allow location access in your browser.</p>
