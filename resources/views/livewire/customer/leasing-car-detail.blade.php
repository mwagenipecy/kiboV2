<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div class="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <a href="{{ route('cars.lease.index') }}" class="flex items-center gap-2 kibo-text font-medium">
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm">
                    ...
                    <div class="flex items-baseline gap-3">
                        <div class="text-4xl font-bold kibo-text">${{ number_format($lease->monthly_payment, 0) }}</div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                        ...
                        <div class="p-4 bg-gray-50 rounded-lg">
                            ...
                            <p class="text-2xl font-bold kibo-text">${{ number_format($lease->total_upfront_cost ?? ($lease->down_payment + ($lease->security_deposit ?? 0)), 0) }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center gap-3">
                            <svg class="w-6 h-6 {{ $lease->maintenance_included ? 'kibo-text' : 'text-gray-400' }}" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <div class="flex items-center gap-3">
                            <svg class="w-6 h-6 {{ $lease->insurance_included ? 'kibo-text' : 'text-gray-400' }}" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                            </svg>
                        ...
                        @if($lease->included_services && count($lease->included_services) > 0)
                            ...
                            <div class="flex items-center gap-3">
                                <svg class="w-6 h-6 kibo-text" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                </svg>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-24 space-y-6" style="z-index: 10;">
                    {{-- Pricing Card --}}
                    <div class="bg-white rounded-xl p-6 shadow-sm border-2 kibo-border relative" style="z-index: 10;">
                        <div class="text-center mb-6">
                            <p class="text-sm text-gray-600 mb-2">Monthly Payment</p>
                            <p class="text-5xl font-bold kibo-text">${{ number_format($lease->monthly_payment, 0) }}</p>
                        ...
                        <div class="space-y-3 mb-6 pb-6 border-b border-gray-200">
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="font-semibold kibo-text">${{ number_format($lease->total_upfront_cost ?? ($lease->down_payment + ($lease->security_deposit ?? 0)), 0) }}</span>
                            </div>
                        </div>
                        ...
                        @auth
                            ...
                            @if($existingOrder)
                                <div class="mb-3 p-4 rounded-lg border-2 
                                    ...
                                    {{ $applicationStatus === 'active' ? 'kibo-bg-light' : '' }}
                                    ...
                                    <div class="flex items-start gap-3">
                                        ...
                                        @elseif($applicationStatus === 'active')
                                            <svg class="w-5 h-5 kibo-text mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                            </svg>
                                            <div class="flex-1">
                                                <h4 class="font-semibold kibo-text mb-1">Lease Active</h4>
                                                <p class="text-sm kibo-text">Your lease (Order #{{ $existingOrder->order_number }}) is currently active. Check your account for lease details and payments.</p>
                                                <p class="text-xs kibo-text mt-2">Started: {{ $existingOrder->order_data['lease_started_at'] ?? $existingOrder->updated_at->format('M d, Y') }}</p>
                                            </div>
                            ...
                            @if($canApply)
                                <button 
                                    ...
                                    class="w-full px-6 py-3 kibo-bg text-white font-bold rounded-lg transition-colors mb-3 hover:opacity-90 active:opacity-75 disabled:opacity-50 disabled:cursor-not-allowed relative z-10"
                                >
                                    <span wire:loading.remove wire:target="openModal">
                                        {{ $existingOrder && $applicationStatus === 'rejected' ? 'Apply Again' : 'Apply for This Lease' }}
                                    </span>
                                    <span wire:loading wire:target="openModal" class="flex items-center justify-center gap-2">
                            ...
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="block w-full px-6 py-3 kibo-bg text-white font-bold rounded-lg text-center transition-colors mb-3">
                                Login to Apply
                            </a>
                        @endauth
                    </div>
                    ...
                    <div class="bg-white rounded-xl p-6 shadow-sm">
                        ...
                        <a href="tel:{{ $lease->entity->phone }}" class="w-full bg-white border-2 kibo-border kibo-text py-3 px-6 rounded-full font-semibold transition-colors flex items-center justify-center gap-2 mb-4">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="mailto:{{ $lease->entity->email }}" class="w-full kibo-bg text-white py-3 px-6 rounded-full font-semibold transition-colors flex items-center justify-center gap-2">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {{-- Leasing Application Modal - Pure Livewire --}}
    @if($showModal)
    <div class="fixed inset-0 z-[9999] overflow-y-auto" wire:key="application-modal">
        <!-- Background overlay -->
        <div class="fixed inset-0 bg-black/50 transition-opacity" wire:click="closeModal"></div>

        <!-- Modal panel -->
        <div class="fixed right-0 top-0 h-full w-full max-w-2xl bg-white shadow-2xl overflow-y-auto z-[10000]">
            <!-- Header -->
            <div class="sticky top-0 bg-gradient-to-r from-[#009866] to-[#007a52] px-6 py-4 z-10">
                <div class="flex items-center justify-between">
                    ...
                    <button wire:click="closeModal" class="text-white hover:opacity-80 transition-colors w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/20">
            ...
            <!-- Content -->
            <form wire:submit.prevent="submitApplication" class="px-6 py-6 space-y-6">
                <!-- Personal Information Section -->
                <div>
                    <h4 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                        <svg class="w-5 h-5 text-[#009866]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" wire:model="full_name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('full_name') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Email Address *</label>
                            <input type="email" wire:model="email" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('email') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Phone Number *</label>
                            <input type="tel" wire:model="phone" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('phone') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Date of Birth *</label>
                            <input type="date" wire:model="date_of_birth" max="{{ date('Y-m-d', strtotime('-18 years')) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('date_of_birth') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Address *</label>
                            <input type="text" wire:model="address" placeholder="Street address" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('address') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">City *</label>
                            <input type="text" wire:model="city" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('city') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Postal Code</label>
                            <input type="text" wire:model="postal_code" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" wire:model="monthly_income" placeholder="0.00" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                        ...
                        <div>
                            ...
                            <select wire:model="employment_status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="employer_name" placeholder="Company name" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                        ...
                        <div>
                            ...
                            <input type="number" wire:model="employment_months" placeholder="0" min="0" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]">
                            @error('employment_months') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div class="flex items-center pt-6">
                            <input type="checkbox" wire:model="current_lease" id="current_lease_modal" class="w-4 h-4 text-[#009866] rounded focus:ring-2 focus:ring-[#009866]">
                            <label for="current_lease_modal" class="ml-2 text-sm font-medium text-gray-700">I currently have an active lease</label>
                        </div>
                    </div>
                </div>
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="file" wire:model="id_document" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                            @error('id_document') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                            @if($id_document)
                            <p class="text-xs text-[#009866] mt-1">✓ File selected: {{ $id_document->getClientOriginalName() }}</p>
                            @endif
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Proof of Income *</label>
                            <input type="file" wire:model="proof_of_income" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                            @error('proof_of_income') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                            @if($proof_of_income)
                            <p class="text-xs text-[#009866] mt-1">✓ File selected: {{ $proof_of_income->getClientOriginalName() }}</p>
                            @endif
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Proof of Address *</label>
                            <input type="file" wire:model="proof_of_address" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                            @error('proof_of_address') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                            @if($proof_of_address)
                            <p class="text-xs text-[#009866] mt-1">✓ File selected: {{ $proof_of_address->getClientOriginalName() }}</p>
                            @endif
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Driving License *</label>
                            <input type="file" wire:model="driving_license" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                            @error('driving_license') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                            @if($driving_license)
                            <p class="text-xs text-[#009866] mt-1">✓ File selected: {{ $driving_license->getClientOriginalName() }}</p>
                            @endif
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Additional Documents (Optional)</label>
                            <input type="file" wire:model="additional_documents" multiple accept=".pdf,.jpg,.jpeg,.png" class="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm">
                            @if(count($additional_documents) > 0)
                            <p class="text-xs text-[#009866] mt-1">✓ {{ count($additional_documents) }} file(s) selected</p>
                            @endif
                        </div>
                ...
                <div class="border-t border-gray-200 pt-6">
                    ...
                    <textarea wire:model="notes" rows="3" placeholder="Any additional information..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#009866] focus:border-[#009866]"></textarea>
                </div>

                <!-- Agreements -->
                <div class="border-t border-gray-200 pt-6 space-y-4">
                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" wire:model="agreeToTerms" class="w-5 h-5 mt-0.5 text-[#009866] rounded focus:ring-2 focus:ring-[#009866]">
                        <span class="text-sm text-gray-700">I agree to the <a href="#" class="text-[#009866] hover:underline">Terms and Conditions</a> and <a href="#" class="text-[#009866] hover:underline">Privacy Policy</a> *</span>
                    </label>
                    @error('agreeToTerms') <span class="text-red-500 text-sm block">{{ $message }}</span> @enderror

                    <label class="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" wire:model="agreeToCreditCheck" class="w-5 h-5 mt-0.5 text-[#009866] rounded focus:ring-2 focus:ring-[#009866]">
                        <span class="text-sm text-gray-700">I authorize a credit check to be performed *</span>
                    </label>
                ...
                <div class="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 -mx-6 flex items-center justify-between gap-4">
                    <button type="button" wire:click="closeModal" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors">
                        Cancel
                    </button>
                    <button type="submit" wire:loading.attr="disabled" wire:target="submitApplication" class="px-8 py-2 bg-[#009866] hover:bg-[#007a52] text-white font-semibold rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
